...
(function (global) {
  ...
  /** 关闭主展区与右侧 Agent，回到首页 Hero */
  function closeWorkspace() {
    if (global.AiAssist && typeof global.AiAssist.hide === "function") {
      try {
        global.AiAssist.hide();
      } catch (err) {}
    }
    clear();
  }
  ...
  function ensureRoot() {
    ...
    if (
      ...
      el.querySelector("#ssWorkspaceClose")
    ) {
      return root;
    }
    root = el;
    ...
    root.innerHTML =
      '<button type="button" class="ss-workspace-close" id="ssWorkspaceClose" aria-label="' +
      esc(t("关闭", "Close")) +
      '">' +
    ...
    var closeBtn = root.querySelector("#ssWorkspaceClose");
    if (closeBtn && closeBtn.dataset.ssBound !== "1") {
      closeBtn.dataset.ssBound = "1";
  ...
  function renderDetail(item) {
    ...
    if (item.kind === "case") return renderCaseDetail(item);
    return renderProductDetail(item);
  }

  function caseMetaPairs(cse) {
    var meta = (cse && cse.meta) || {};
    var keys = Object.keys(meta);
    var out = [];
    keys.forEach(function (k) {
      if (meta[k]) out.push({ label: k, value: String(meta[k]) });
    });
    return out;
  }

  function caseSummaryCards(cse) {
    var sum = (cse && cse.summary) || {};
    return [
      { key: "challenge", section: sum.challenge, fallback: t("挑战", "Challenge") },
      { key: "solution", section: sum.solution, fallback: t("解决方案", "Solution") },
      { key: "result", section: sum.result, fallback: t("结果", "Result") },
    ];
  }
  ...
  function renderCaseDetail(item) {
    ...
    var main =
      ...
      (item.cover_url ? { url: item.cover_url, media_type: "image" } : null);
    var mainImg = main && main.url ? main.url : "";
    var lead = String(item.description || "").trim();
    var meta = caseMetaPairs(cse);
    var cards = caseSummaryCards(cse);
    var detail = cse.detail || {};
    var detailOrder = [
      { key: "customer", section: detail.customer },
      { key: "challenge", section: detail.challenge },
      { key: "solution", section: detail.solution },
      { key: "result", section: detail.result },
    ];
    ...
    html += '<div class="ss-case-body">';
    html += '<h1 class="ss-case-title">' + esc(item.name) + "</h1>";
    if (lead) {
      html +=
        '<p class="ss-case-lead">' +
        esc(lead).replace(/\n/g, "<br />") +
        "</p>";
    }
    html += '<div class="ss-product-accent" aria-hidden="true"></div>';
    ...
    if (meta.length) {
      html += '<div class="ss-case-meta">';
      meta.forEach(function (m) {
        html +=
          '<div class="ss-case-meta-card">' +
          '<div class="ss-case-meta-label">' +
          esc(m.label) +
          "</div>" +
          '<div class="ss-case-meta-value">' +
          esc(m.value) +
          "</div></div>";
      });
      html += "</div>";
    ...
    var hasSummary = cards.some(function (c) {
      return c.section && c.section.bullets && c.section.bullets.length;
    });
    if (hasSummary) {
      html += '<section class="ss-case-summary">';
      cards.forEach(function (c) {
        var sec = c.section || {};
        var bullets = sec.bullets || [];
        if (!bullets.length) return;
        html +=
          '<article class="ss-case-card">' +
          '<h2 class="ss-case-card-title">' +
          esc(sec.title || c.fallback) +
          "</h2>" +
          renderList(bullets) +
          "</article>";
      });
    ...
    detailOrder.forEach(function (row) {
      var sec = row.section || {};
      if (!sec.body && !sec.subtitle) return;
      html +=
        '<section class="ss-case-detail-block">' +
        '<h2 class="ss-case-detail-title">' +
        esc(sec.title || "") +
        "</h2>";
      if (sec.subtitle) {
        html +=
          '<h3 class="ss-case-detail-sub">' + esc(sec.subtitle) + "</h3>";
      }
      if (sec.body) {
        html +=
          '<div class="ss-case-detail-body">' +
