...
(function () {
  ...
  var DEFAULTS = {
     ...
     * 0 = 强制 VPS 方案
     * 1 = 强制 CF 方案
     * 2 = 自动（按访客地区：中国大陆→CF，其它→VPS）
     */
    llmRouteMode: 2,
    /**
     * 路由地区模拟（仅 llmRouteMode=2 自动时生效；本机测国内选路用）：
     * 0=关闭（用真实 cf.country）/ 1=模拟中国大陆CN / 2=模拟美国US
     */
    llmRouteDebugCountry: 0,
    /**
     * 企业问答新问题通知邮箱（逗号/分号分隔）。
     * 与 Cloudflare env AGENT_QA_NOTIFY_EMAILS 合并；需 RESEND_API_KEY。
     */
    agentQaNotifyEmails: "",
  };
  ...
  var PARAM_ROWS = [
    ...
    },
    {
      category: "AI助手",
      field: "llmRouteMode",
      desc: "路由：0=强制VPS / 1=强制CF / 2=自动(按地区)",
      value: 2,
      unit: "0/1/2",
    },
    {
      category: "AI助手",
      field: "llmRouteDebugCountry",
      desc: "路由模拟：0关 / 1模拟CN / 2模拟US（仅自动模式；测完改回0）",
      value: 0,
      unit: "0/1/2",
    },
    {
      category: "企业问答",
      field: "agentQaNotifyEmails",
      desc: "新问题邮件通知：多个邮箱用逗号分隔（需已配 RESEND）",
      value: "",
      unit: "邮箱",
    },
  ];
  ...
  var template =
    ...
    '<div id="settingsGrid" class="ag-theme-quartz" style="height:min(56vh,520px);width:100%;"></div>' +
  ...
  function mergeSettings(saved) {
    ...
    out.llmRouteDebugCountry = clampInt(
      out.llmRouteDebugCountry,
      0,
      2,
      DEFAULTS.llmRouteDebugCountry
    );
    out.agentQaNotifyEmails = String(
      out.agentQaNotifyEmails != null
        ? out.agentQaNotifyEmails
        : DEFAULTS.agentQaNotifyEmails
    ).slice(0, 2000);
    return out;
  }
  ...
  function collectFromGrid() {
    ...
    return {
      ...
      llmRouteMode: clampInt(raw.llmRouteMode, 0, 2, DEFAULTS.llmRouteMode),
      llmRouteDebugCountry: clampInt(
        raw.llmRouteDebugCountry,
        0,
        2,
        DEFAULTS.llmRouteDebugCountry
      ),
      ...
      agentQaNotifyEmails: String(
        raw.agentQaNotifyEmails != null
          ? raw.agentQaNotifyEmails
          : DEFAULTS.agentQaNotifyEmails
      )
        .replace(/[^\w.@+\-_,;\s]/g, "")
        .slice(0, 2000),
