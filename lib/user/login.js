...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            /** sessionId → true：空字段已提示过，避免轮询刷屏 */
            var registerEmptyFieldsAlertedForSession = Object.create(null);
            ...
            async function handleScannedResult(sessionId, gen) {
                ...
                if (result.exists) {
                    ...
                    if (!phone || !email) {
                        if (!registerEmptyFieldsAlertedForSession[sessionId]) {
                            registerEmptyFieldsAlertedForSession[sessionId] = true;
                            setRegisterLoginStatus('code', '手机号、邮箱不能为空！', 'error');
                            alert('手机号、邮箱不能为空！');
                        }
                        return;
                    }
                    ...
                    console.log(
                        '%c【扫码成功】%c 手机已确认，准备人机验证后发送登录确认邮件',
                    ...
                    setRegisterLoginStatus('code', '已收到手机确认，请完成人机验证…', 'pending');

                    function resumeScanPollAfterTurnstileCancel() {
                        registerEmailCodeSending = false;
                        delete registerEmailSendStartedForSession[sessionId];
                        setRegisterLoginStatus('code', '已取消人机验证，可重新完成验证', 'error');
                        if (isRegisterPanelLive(myGen)) startPolling(sessionId);
                    }
                    ...
                    requireTurnstileThen(
                        async function () {
                            ...
                            setRegisterLoginStatus(
                                'code',
                                '确认邮件已发送，请到邮箱打开确认页并点击「确认登录」',
                                'ok'
                            );
                            startEmailLoginConfirmPoll(sessionId, myGen);
                        },
                        resumeScanPollAfterTurnstileCancel
                    );
                } else {
            ...
            function refreshCameraQr() {
                ...
                registerEmptyFieldsAlertedForSession = Object.create(null);
            ...
            function beginRegisterFlow() {
                // 人机验证：密码登录点「登录」前；扫码登录在收到手机「确认登录」后、发信前
                openRegister();
            }
            ...
            function startEmailLoginConfirmPoll(sessionId, gen) {
                ...
                async function tick() {
                    ...
                    if (ticks > 200) {
                        stopEmailLoginConfirmPoll();
                        setRegisterLoginStatus('code', '等待邮件确认超时，请重新扫码', 'error');
                        return;
                    }
                    try {
                        ...
                        if (
                            ...
                            registerEmailVerificationBind = {
                                phone: String(d.phone || (registerEmailVerificationBind && registerEmailVerificationBind.phone) || '').trim(),
                                email: String(d.email || (registerEmailVerificationBind && registerEmailVerificationBind.email) || '').trim(),
                                username: String(d.username || (registerEmailVerificationBind && registerEmailVerificationBind.username) || '').trim()
                            };
                            setRegisterLoginStatus('code', '邮箱已确认，正在登录…', 'pending');
            ...
            async function completeRegisterCodeLoginFromVerifiedEmail() {
                ...
                var phone = String(bind.phone || '').trim();
                var email = String(bind.email || '').trim();
                ...
                if (!phone || !email) {
                    setRegisterLoginStatus(
                        'code',
                        '确认会话缺少账号信息，请重新扫码并查收邮件',
                        'error'
                    );
                    alert('确认会话缺少账号信息，请重新扫码。');
                    return;
                }
