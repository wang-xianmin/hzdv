...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            async function registerSendEmailCodeHandler() {
                ...
                // 一生成就打到 Console（不依赖邮件是否发出），扫码自测可直接用
                logRegisterVerificationCodeToConsole(newCode, email, '刚生成 · 正在发信');
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        console.error('[验证码登录] 发送失败（验证码仍可在上方 Console 查看）:', errorText);
                        logRegisterVerificationCodeToConsole(newCode, email, '发信失败 · 调试仍可用');
                        var failDetail = '';
                        try {
                        ...
                        setRegisterLoginStatus(
                            'code',
                            failMsg + '；可在 Console 查看验证码继续填写',
                            'error'
                        );
                        alert(
                            failMsg +
                                '\n\n验证码已打到浏览器 Console（搜索「登录验证码」），可直接填写继续自测。'
                        );
                        return true;
                    }
                    registerEmailVerificationCode = newCode;
                    ...
                    logRegisterVerificationCodeToConsole(newCode, email, '发信成功 · 与邮件正文一致');
                    setRegisterLoginStatus(
                        'code',
                        '验证码已发送；未收到邮件请按 F12 在 Console 查看',
                        'ok'
                    ...
                    logRegisterVerificationCodeToConsole(newCode, email, '网络异常 · 调试仍可用');
                    registerEmailVerificationCode = newCode;
                    registerEmailVerificationBind = {
                        phone: phone,
                        email: email
                    ...
                    setRegisterLoginStatus(
                        'code',
                        '发送验证码网络异常；可在 Console 查看验证码继续填写',
                        'error'
                    );
                    alert(
                        '网络异常，邮件可能未发出。\n验证码已打到 Console（搜索「登录验证码」），可直接填写继续自测。'
                    );
                    return true;
                }
            }
