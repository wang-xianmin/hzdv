...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            async function handleScannedResult(sessionId, gen) {
                ...
                if (result.exists && result.data && result.data.scanned === true) {
                    ...
                    // 其他 pcStatus（processing/reject_*）已有处理方处理
            ...
            var registerQrPollInFlight = false;
            function startPolling(sessionId) {
                ...
                window.qrTimer = setInterval(function () {
                    ...
                    if (registerQrPollInFlight) return;
                    registerQrPollInFlight = true;
                    handleScannedResult(sessionId, pollGen).finally(function () {
                        registerQrPollInFlight = false;
                    });
                }, 350);
            ...
            function refreshCameraQr() {
                ...
                // 预写会话到 KV（含 phone/email），手机扫码时服务端可直接发信，无需等电脑端二次轮询
                var _prePhone = document.getElementById('registerCodePhoneInput');
                var _preEmail = document.getElementById('registerCodeEmailInput');
                fetch('/api/scan-login', {
                    ...
                    body: JSON.stringify({
                        ...
                        data: {
                            scanned: false,
                            createdAt: Date.now(),
                            phone: _prePhone ? _prePhone.value.trim() : '',
                            email: _preEmail ? _preEmail.value.trim() : '',
            ...
            function generateQrCode(baseUrl, sessionId) {
                ...
                // 生成扫码 URL（带 lang + phone + email，手机扫码时直接传给服务端发信，绕过 KV 一致性延迟）
                var qrLang =
                    typeof loginUiLang === 'function'
                        ? loginUiLang()
                        : String(window.currentLang || 'zh');
                var _qrPhone = _prePhone ? _prePhone.value.trim() : '';
                var _qrEmail = _preEmail ? _preEmail.value.trim() : '';
                var scanUrl =
                    baseUrl +
                    ...
                    (_qrPhone ? '&p=' + encodeURIComponent(_qrPhone) : '') +
                    (_qrEmail ? '&e=' + encodeURIComponent(_qrEmail) : '');
