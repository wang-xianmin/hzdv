...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            async function handleScannedResult(sessionId, gen) {
                ...
                if (!isRegisterPanelLive(myGen)) return;
                // 只要 KV 已写入即视为扫码成功（手机端须先过 Turnstile 才会写入）
                if (result.exists) {
                    clearInterval(window.qrTimer); // 停止轮询
                    window.qrTimer = null;
                    registerQrPollLogCount = 0;
                    console.log(
                        '%c【扫码成功】%c 会话已写入，准备发送登录确认邮件',
                    ...
                    setRegisterLoginStatus('code', '已检测到扫码，正在发送登录确认邮件…', 'pending');
                    if (!phone || !email) {
            ...
            function beginRegisterFlow() {
                // 人机验证改到：密码登录点「登录」前；扫码登录在手机端勾选后才回写 KV
                openRegister();
            }
            ...
            function requireTurnstileThen(onPass) {
                ...
                openTurnstileOverlay(siteKey, cfg, function (token) {
                    onPass(token || null);
                });
            }
            ...
            function startEmailLoginConfirmPoll(sessionId, gen) {
                ...
                async function tick() {
                    ...
                    try {
                        ...
                        if (dPoll && dPoll.emailLoginConfirmed) {
                            stopEmailLoginConfirmPoll();
                            registerScanPhoneMismatchCount = 0;
                            var d = dPoll;
