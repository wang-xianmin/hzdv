...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            /** sessionId → true：该会话已开始发确认邮件（防轮询并发双发） */
            var registerEmailSendStartedForSession = Object.create(null);

            function isValidLoginEmail(s) {
                var e = String(s == null ? '' : s).trim();
                if (!e || e.indexOf('@') < 1) return false;
                return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e);
            }

            function loginUiLang() {
                var L = window.currentLang || document.documentElement.lang || 'zh';
                if (String(L).toLowerCase().indexOf('en') === 0) return 'en';
                return 'zh';
            }

            function tScan(zh, en) {
                return loginUiLang() === 'en' ? en : zh;
            }
            var registerEmailConfirmTimer = null;
            var registerEmailConfirmSessionId = null;
            /** 关闭/重开登录窗时递增，作废进行中的扫码轮询回调，避免关不掉又被拉起 */
            var registerUiGeneration = 0;
            var registerPwdNotFoundCount = 0;
            var registerPwdWrongCount = 0;
            /** 扫码登录：邮件确认时手机号/邮箱与 KV 不一致的连续次数，满 2 次关窗 */
            var registerScanPhoneMismatchCount = 0;
            var registerLastHandledPhoneMismatchAt = 0;
            ...
            var turnstileOnCancel = null;
            /** 强制人机验证：隐藏取消、禁止点遮罩关闭（扫码邮件确认后） */
            var turnstileForceMode = false;
            /** 密码登录页内联 Turnstile（非弹层） */
            var pwdTurnstileMount = document.getElementById('registerPwdTurnstileMount');
            var pwdInlineWidgetId = null;
            var pwdInlineToken = null;
            ...
            function getRegisterCodeStep() {
                var panel = registerTabPanels.code || document.getElementById('registerTabCode');
                return panel && panel.getAttribute('data-code-step') === 'qr' ? 'qr' : 'form';
            }

            function setRegisterCodeStep(step) {
                var panel = registerTabPanels.code || document.getElementById('registerTabCode');
                var next = step === 'qr' ? 'qr' : 'form';
                if (panel) panel.setAttribute('data-code-step', next);
                var phoneLine = document.getElementById('registerCodePhoneLine');
                var continueBtn = document.getElementById('registerCodeContinueBtn');
                var scanCol = document.getElementById('registerScanCol');
                var backBtn = document.getElementById('registerCodeBackBtn');
                var emailEl = document.getElementById('registerCodeEmailInput');
                if (phoneLine) phoneLine.hidden = next === 'qr';
                if (continueBtn) continueBtn.hidden = next === 'qr';
                if (scanCol) scanCol.hidden = next !== 'qr';
                if (backBtn) {
                    backBtn.hidden = next !== 'qr';
                    if (next === 'qr') backBtn.removeAttribute('hidden');
                    else backBtn.setAttribute('hidden', '');
                }
                if (emailEl) {
                    if (next === 'qr') {
                        emailEl.setAttribute('disabled', 'disabled');
            ...
            function switchRegisterTab(name) {
                ...
                /** 密码登录 Tab 下停止 scan-login 轮询；切回扫码且已进入二维码步时恢复 */
                if (name === 'pwd') {
                    window.stopPolling();
                    ensurePwdInlineTurnstile();
                } else if (name === 'code') {
                    if (
                        registerPanel &&
                        registerPanel.classList.contains('show') &&
                        getRegisterCodeStep() === 'qr' &&
                        cameraQrImg &&
                        cameraQrImg.dataset.sessionId
                    ) {
            ...
            // 这也是轮询检查扫码结果
            var registerQrPollLogCount = 0;
            function isRegisterPanelLive(gen) {
                return (
                    registerPanel &&
                    registerPanel.classList.contains('show') &&
                    (gen == null || gen === registerUiGeneration)
                );
            }
            async function patchScanSession(sessionId, patch) {
                if (!sessionId || !patch || typeof patch !== 'object') return;
                try {
                    await fetch('/api/scan-login', {
                        method: 'POST',
                    ...
                    console.warn('回写扫码会话失败:', ePatch);
                }
            }

            async function handleScannedResult(sessionId, gen) {
                var myGen = gen == null ? registerUiGeneration : gen;
                try {
                if (!isRegisterPanelLive(myGen)) return;
                ...
                if (!isRegisterPanelLive(myGen)) return;
                // 手机打开扫码链接写入 KV 后，主站直接发确认邮件（真人验证改到邮件确认之后）
                if (result.exists && result.data) {
                    var scanData = result.data || {};
                    // 服务端已处理完（triggered by scan-login POST）
                    if (scanData.pcStatus === 'ok' && scanData.emailSent) {
                        registerEmailSendStartedForSession[sessionId] = true;
                        ...
                        setRegisterLoginStatus(
                            ...
                            tScan(
                                '确认邮件已发送，请在邮箱中点击确认登录',
                                'Confirmation email sent. Please confirm sign-in in your email.'
                            ),
                            'ok'
                        );
                        startEmailLoginConfirmPoll(sessionId, myGen);
                        return;
                    }
                    if (scanData.pcStatus === 'processing') {
                        setRegisterLoginStatus(
                            'code',
                            tScan('正在发送确认邮件…', 'Sending confirmation email…'),
                            'pending'
                        );
                        return;
                    }
                    // 其他 pcStatus（reject_* 等）已有处理方处理
                    if (scanData.pcStatus) return;
                    if (registerEmailSendStartedForSession[sessionId] || registerEmailCodeSending) {
                        return;
                    }
                    ...
                    if (!phone || !email || !isValidLoginEmail(email)) {
                        // 先立刻提示电脑端，再回写拒绝供手机端收敛（勿提示查收邮箱）
                        registerEmailSendStartedForSession[sessionId] = true;
                        clearInterval(window.qrTimer);
                        window.qrTimer = null;
                        var emptyIsFormat = !!(phone && email && !isValidLoginEmail(email));
                        var emptyMsg =
                            emptyIsFormat
                                ? tScan(
                                      '邮箱格式不正确，请重新填写后再扫码。',
                                      'Invalid email format. Please correct it and scan again.'
                                  )
                                : tScan(
                                      '手机号、邮箱不能为空！',
                                      'Phone and email are required!'
                                  );
                        setRegisterLoginStatus('code', emptyMsg, 'error');
                        alert(emptyMsg);
                        await patchScanSession(sessionId, {
                            pcStatus: emptyIsFormat ? 'reject_email' : 'reject_empty',
                            rejectMsgZh: emptyIsFormat
                                ? '邮箱格式不正确，请重新填写后再扫码。'
                                : '手机号、邮箱不能为空！',
                            rejectMsgEn: emptyIsFormat
                                ? 'Invalid email format. Please correct it and scan again.'
                                : 'Phone and email are required!',
                            awaitingPc: false
                        });
                        if (isRegisterPanelLive(myGen)) refreshCameraQr();
                        return;
                    }
                    ...
                    clearInterval(window.qrTimer);
                    window.qrTimer = null;
                    registerQrPollLogCount = 0;
                    console.log(
                        '%c【扫码成功】%c 手机已打开链接，准备发送登录确认邮件',
                        'background:#161823;color:#fff;font-size:12px;padding:3px 8px;border-radius:4px;',
                        'color:#0b7285;font-size:12px;'
                    );

                    registerEmailCodeSending = true;
                    // 立刻反馈（processing/ok 由发信 API 写入会话，避免客户端二次回写抢状态）
                    setRegisterLoginStatus(
                        'code',
                        tScan('正在发送确认邮件…', 'Sending confirmation email…'),
                        'pending'
                    );
                    var sentOk = await registerSendEmailCodeHandler(sessionId);
                    if (!isRegisterPanelLive(myGen)) {
                        registerEmailCodeSending = false;
                        return;
                    }
                    if (!sentOk) {
                        registerEmailCodeSending = false;
                        // 服务端失败时通常已写 reject_send；此处兜底，不阻塞刷新
                        patchScanSession(sessionId, {
                        ...
                        delete registerEmailSendStartedForSession[sessionId];
                        refreshCameraQr();
                        return;
                    }
                    registerEmailCodeSending = false;
                    if (cameraQrImg) cameraQrImg.style.opacity = '0.3';
                    // 与原先一致：发信 API 返回后立刻提示；ok 已由服务端写入会话
                    setRegisterLoginStatus(
                        'code',
                        tScan(
                            '确认邮件已发送，请在邮箱中点击确认登录',
                    ...
                    registerQrPollLogCount += 1;
                    // 降低刷屏：第 1 次、之后每 20 次打一条
                    if (registerQrPollLogCount === 1 || registerQrPollLogCount % 20 === 0) {
                        console.log(
                            '轮询中: 会话尚未写入（等手机打开扫码页）',
                            sessionId,
                            '×' + registerQrPollLogCount
                        );
                    }
                }
                } catch (error) {
            ...
            function startPolling(sessionId) {
                ...
                registerQrPollLogCount = 0;
                if (!isRegisterPanelLive()) return;
                var pollGen = registerUiGeneration;

                // 先建立定时器，再立即执行一次检查（确保 handleScannedResult 内能停止轮询）
                window.qrTimer = setInterval(async function () {
                    if (!isRegisterPanelLive(pollGen)) {
                        clearInterval(window.qrTimer);
                        window.qrTimer = null;
                        return;
                    }
                    ...
                    if (getRegisterCodeStep() !== 'qr') {
                        return;
                    }
                    await handleScannedResult(sessionId, pollGen);
                }, 250);
                
                // 立即执行一次检查
                handleScannedResult(sessionId, pollGen);
            }
            ...
            function generateQrCode(baseUrl, sessionId) {
                ...
                // 生成扫码 URL（带 lang、手机号、邮箱；手机扫码后服务端可直接发确认邮件）
                var qrLang =
                    typeof loginUiLang === 'function'
                        ? loginUiLang()
                        : String(window.currentLang || 'zh');
                var qrPhoneEl = document.getElementById('registerCodePhoneInput');
                var qrEmailEl = document.getElementById('registerCodeEmailInput');
                var qrPhone = qrPhoneEl ? String(qrPhoneEl.value || '').trim() : '';
                var qrEmail = qrEmailEl ? String(qrEmailEl.value || '').trim() : '';
                var scanUrl =
                    baseUrl +
                    '/mobile.html?sessionId=' +
                    encodeURIComponent(sessionId) +
                    '&lang=' +
                    encodeURIComponent(qrLang);
                if (qrPhone) scanUrl += '&p=' + encodeURIComponent(qrPhone);
                if (qrEmail) scanUrl += '&e=' + encodeURIComponent(qrEmail);
            ...
            function closeTurnstileOverlay(asCancel) {
                if (!turnstileOverlay) return;
                if (turnstileForceMode && asCancel) {
                    // 强制模式不允许取消关闭
                    return;
                }
                ...
                turnstileOverlay.classList.remove('turnstile-overlay--force');
                turnstileOverlay.setAttribute('aria-hidden', 'true');
                turnstileForceMode = false;
                if (turnstileCancelBtn) {
                    turnstileCancelBtn.hidden = false;
                    turnstileCancelBtn.style.display = '';
                }
                var cancelCb = turnstileOnCancel;
                turnstileOnCancel = null;
                if (asCancel && typeof cancelCb === 'function') {
                    try {
                        cancelCb();
                    } catch (eCancel) {}
                }
             ...
             * @param options.keepOpen 为 true 时不关弹层（密码登录内联校验用）
             */
            function verifyTurnstileToken(token, cfg, onVerified, skipVerifyEndpoint, options) {
                options = options || {};
                function finishOk() {
                    if (!options.keepOpen) {
                        turnstileOnCancel = null;
                ...
                fetch(endpoint, {
                    ...
                    .then(function (x) {
                        if (x.j && x.j.success) {
                            ...
                            alert('验证未通过，请重试');
                            if (options.keepOpen) {
                                resetPwdInlineTurnstile();
                            } else if (turnstileWidgetId !== null && window.turnstile) {
                    ...
                    .catch(function (err) {
                        ...
                        if (options.keepOpen) resetPwdInlineTurnstile();
                    });
            }

            function destroyPwdInlineTurnstile() {
                pwdInlineToken = null;
                if (pwdInlineWidgetId !== null && window.turnstile) {
                    try {
                        window.turnstile.remove(pwdInlineWidgetId);
                    } catch (e) { /* ignore */ }
                    pwdInlineWidgetId = null;
                }
                if (pwdTurnstileMount) pwdTurnstileMount.innerHTML = '';
            }

            function resetPwdInlineTurnstile() {
                pwdInlineToken = null;
                if (pwdInlineWidgetId !== null && window.turnstile) {
                    try {
                        window.turnstile.reset(pwdInlineWidgetId);
                    } catch (e) { /* ignore */ }
                }
            }

            /** 密码登录 Tab：在密码框下渲染内联 Turnstile（含 CF 标识） */
            function ensurePwdInlineTurnstile() {
                if (!pwdTurnstileMount) return;
                if (shouldBypassTurnstileForLogin()) {
                    destroyPwdInlineTurnstile();
                    pwdTurnstileMount.hidden = true;
                    return;
                }
                ...
                if (!siteKey) {
                    ...
                    pwdTurnstileMount.hidden = false;
                    return;
                }
                pwdTurnstileMount.hidden = false;
                if (pwdInlineWidgetId !== null) return;
                ensureTurnstileScript()
                    .then(function () {
                        if (pwdInlineWidgetId !== null || !pwdTurnstileMount) return;
                        pwdInlineToken = null;
                        pwdInlineWidgetId = window.turnstile.render(pwdTurnstileMount, {
                            sitekey: siteKey,
                            ...
                            callback: function (token) {
                                pwdInlineToken = token || null;
                                setRegisterLoginStatus('pwd', '', '');
                            },
                            'error-callback': function (errorCode) {
                                pwdInlineToken = null;
                                console.warn('Turnstile 内联错误, code:', errorCode);
                                var detail = errorCode ? '错误码: ' + String(errorCode) : '';
                                showTurnstileDiagnosticDialog(
                                    'Turnstile 组件返回错误 (code: ' + (errorCode || 'unknown') + ')',
                                    cfg,
                                    detail
                                );
                            },
            ...
            function openTurnstileOverlay(siteKey, cfg, onVerified, skipVerifyEndpoint, onCancel, options) {
                if (!turnstileOverlay || !turnstileWidgetMount) return;
                options = options || {};
                turnstileForceMode = !!options.force;
                turnstileOnCancel = turnstileForceMode
                    ? null
                    : typeof onCancel === 'function'
                      ? onCancel
                      : null;
                turnstileOverlay.classList.add('show');
                turnstileOverlay.classList.toggle('turnstile-overlay--force', turnstileForceMode);
                turnstileOverlay.setAttribute('aria-hidden', 'false');
                if (turnstileCancelBtn) {
                    if (turnstileForceMode) {
                        turnstileCancelBtn.hidden = true;
                        turnstileCancelBtn.style.display = 'none';
                    } else {
                        turnstileCancelBtn.hidden = false;
                        turnstileCancelBtn.style.display = '';
                    }
                ...
                ensureTurnstileScript()
                    ...
                    .catch(function (e) {
                        console.error(e);
                        showTurnstileDiagnosticDialog('无法加载 Turnstile 脚本', cfg, e && e.message);
                        if (!turnstileForceMode) closeTurnstileOverlay(true);
                    });
            }
                ...
                fetch('/api/turnstile-config', { method: 'GET', cache: 'no-store' })
                    ...
                    .then(function (data) {
                        if (data && data.success && data.siteKey) {
                            ...
                            var pwdPanel = document.getElementById('registerTabPwd');
                            if (pwdPanel && pwdPanel.classList.contains('active')) {
                                destroyPwdInlineTurnstile();
                                ensurePwdInlineTurnstile();
                            }
                        }
            ...
            /* 系统参数设置已迁至 lib/user/system-settings-modal.js */
            ...
            function openUserListModal() {
                ...
                if (typeof window.userCanSeeOpsUserList === 'function' && !window.userCanSeeOpsUserList()) {
                    alert('无用户列表权限');
                    return;
                }
            ...
            function newUserKvNeedsTurnstile() {
                ...
                /** 显式 false 才跳过；未配置时与服务端一致默认需要 */
                if (cfg.requireForNewUserKv === false) return false;
            ...
            if (registerDebugBypassTurnstile) {
                registerDebugBypassTurnstile.addEventListener('change', function () {
                    ...
                    if (pwdPanel && pwdPanel.classList.contains('active')) {
                        if (this.checked) {
                            destroyPwdInlineTurnstile();
                            if (pwdTurnstileMount) pwdTurnstileMount.hidden = true;
                        } else {
                            destroyPwdInlineTurnstile();
                            ensurePwdInlineTurnstile();
                        }
            ...
            if (turnstileCancelBtn && turnstileOverlay) {
                turnstileCancelBtn.addEventListener('click', function () {
                    if (turnstileForceMode) return;
                    closeTurnstileOverlay(true);
                });
                turnstileOverlay.addEventListener('click', function (e) {
                    if (e.target === turnstileOverlay) {
                        if (turnstileForceMode) return;
                        closeTurnstileOverlay(true);
                    }
                });
            ...
            function shouldBypassTurnstileForLogin() {
                if (isAppDevDebugEnabled()) {
                    var bypassEl = document.getElementById('registerDebugBypassTurnstile');
                    if (bypassEl && bypassEl.checked) return true;
                    if (sessionStorage.getItem('L_ENG_debug_bypass_turnstile') === '1') return true;
                }
                return false;
            }

            function requireTurnstileThen(onPass, onCancel, options) {
                if (typeof onPass !== 'function') return;
                options = options || {};
                if (shouldBypassTurnstileForLogin()) {
                    onPass(null);
                    return;
                ...
                if (!siteKey) {
                    ...
                    showTurnstileDiagnosticDialog(
                        '未配置 Turnstile 站点密钥',
                        cfg,
                        '请配置 Pages 环境变量 TURNSTILE_SITE_KEY'
                    );
                    if (!options.force && typeof onCancel === 'function') onCancel();
                    return;
                }
                openTurnstileOverlay(
                    siteKey,
                    ...
                    false,
                    options.force ? null : typeof onCancel === 'function' ? onCancel : null,
                    options
                );
            }
            ...
            if (registerSubmitBtn) {
                registerSubmitBtn.addEventListener('click', function () {
                    ...
                    function startPwdLogin() {
                        runPasswordLoginAfterTurnstile(phone, password);
                    }
                    ...
                    ensurePwdInlineTurnstile();
                    if (!pwdInlineToken) {
                        setRegisterLoginStatus('pwd', '请先完成下方的人机验证', 'pending');
                        return;
                    }
                    var token = pwdInlineToken;
                    pwdInlineToken = null;
                    setRegisterLoginStatus('pwd', '人机验证中…', 'pending');
            ...
            async function runPasswordLoginAfterTurnstile(phone, password) {
                    if (registerPwdSubmitting) return;
                    registerPwdSubmitting = true;
                    var oldPwdBtnText = registerSubmitBtn ? registerSubmitBtn.textContent : '登录';
                    if (registerSubmitBtn) {
            ...
            if (registerCloseBtn) {
                registerCloseBtn.addEventListener('click', function (ev) {
                    if (ev) {
                        ev.preventDefault();
                        ev.stopPropagation();
                    }
            ...
            function onRegisterCodeContinue() {
                var phoneEl = document.getElementById('registerCodePhoneInput');
                var emailEl = document.getElementById('registerCodeEmailInput');
                ...
                if (!isValidLoginEmail(email)) {
                    ...
                    setRegisterLoginStatus('code', fmtMsg, 'error');
                    alert(fmtMsg);
                    return;
                }
                setRegisterLoginStatus('code', '', '');
                setRegisterCodeStep('qr');
                refreshCameraQr();
            }

            function onRegisterCodeBack() {
                try {
                    window.stopPolling();
                } catch (eStop) {}
                if (window.qrTimer) {
                ...
                stopRegisterQrCountdownTimers();
                stopEmailLoginConfirmPoll();
                registerEmailCodeSending = false;
                registerEmailSendStartedForSession = Object.create(null);
                if (cameraQrImg) {
                    cameraQrImg.removeAttribute('src');
                    delete cameraQrImg.dataset.sessionId;
                    cameraQrImg.style.opacity = '1';
                }
                clearRegisterQrCountdownUI();
                setRegisterLoginStatus('code', '', '');
                setRegisterCodeStep('form');
                var phoneEl = document.getElementById('registerCodePhoneInput');
                if (phoneEl) {
                    try {
                        phoneEl.focus();
            ...
            var registerCodeContinueBtn = document.getElementById('registerCodeContinueBtn');
            var registerCodeBackBtn = document.getElementById('registerCodeBackBtn');
            if (registerCodeContinueBtn) {
                registerCodeContinueBtn.addEventListener('click', onRegisterCodeContinue);
            }
            if (registerCodeBackBtn) {
                registerCodeBackBtn.addEventListener('click', onRegisterCodeBack);
            }
            ...
            function stopEmailLoginConfirmPoll() {
                if (registerEmailConfirmTimer) {
                    clearInterval(registerEmailConfirmTimer);
                    registerEmailConfirmTimer = null;
                }
                registerEmailConfirmSessionId = null;
            }

            function startEmailLoginConfirmPoll(sessionId, gen) {
                stopEmailLoginConfirmPoll();
                registerEmailConfirmSessionId = sessionId;
                var myGen = gen == null ? registerUiGeneration : gen;
                var ticks = 0;
                async function tick() {
                    if (!isRegisterPanelLive(myGen)) {
                    ...
                    if (ticks > 200) {
                        ...
                        setRegisterLoginStatus(
                            ...
                            tScan(
                                '等待邮件确认超时，请重新扫码',
                                'Timed out waiting for email confirmation. Please scan again.'
                            ),
                            'error'
                        );
                        return;
                    ...
                    try {
                        ...
                        var result = await response.json();
                        var dPoll = (result && result.exists && result.data) || null;
                        if (
                            dPoll &&
                            dPoll.emailLoginPhoneMismatch &&
                            dPoll.phoneMismatchAt &&
                            Number(dPoll.phoneMismatchAt) !== registerLastHandledPhoneMismatchAt
                        ) {
                            registerLastHandledPhoneMismatchAt = Number(dPoll.phoneMismatchAt);
                            stopEmailLoginConfirmPoll();
                            registerScanPhoneMismatchCount += 1;
                            setRegisterLoginStatus('code', '手机号错！', 'error');
                            alert('手机号错！');
                            if (registerScanPhoneMismatchCount >= 2) {
                                alert('手机号连续两次错误，已关闭登录窗口。');
                                registerScanPhoneMismatchCount = 0;
                                closeRegister();
                                return;
                            }
                            registerEmailCodeSending = false;
                        ...
                        if (
                            dPoll &&
                            dPoll.emailLoginConfirmed &&
                            dPoll.confirmMethod === 'post'
                        ) {
                            stopEmailLoginConfirmPoll();
                            registerScanPhoneMismatchCount = 0;
                            var d = dPoll;
                            var winPhoneEl = document.getElementById('registerCodePhoneInput');
                            var winEmailEl = document.getElementById('registerCodeEmailInput');
                            var winPhone = winPhoneEl ? winPhoneEl.value.trim() : '';
                            var winEmail = winEmailEl ? winEmailEl.value.trim() : '';
                            registerEmailVerificationBind = {
                                phone: winPhone || String(d.phone || '').trim(),
                                email: winEmail || String(d.email || '').trim(),
                                username: String(d.username || (registerEmailVerificationBind && registerEmailVerificationBind.username) || '').trim()
                            };
                            setRegisterLoginStatus(
                                'code',
                                tScan('邮箱已确认，请完成人机验证…', 'Email confirmed. Please complete the human check…'),
                                'pending'
                            );
                            requireTurnstileThen(
                                async function () {
                                    if (!isRegisterPanelLive(myGen)) return;
                                    setRegisterLoginStatus(
                                        'code',
                                        tScan(
                                            '人机验证通过，正在校验账号…',
                                            'Human check passed. Verifying account…'
                                        ),
                                        'pending'
                                    );
                                    await completeRegisterCodeLoginFromVerifiedEmail();
                                },
                                null,
                                { force: true }
                            );
                        ...
                        console.warn('邮件确认轮询失败:', err);
                    }
                }
                registerEmailConfirmTimer = setInterval(tick, 2000);
                tick();
            }

            async function registerSendEmailCodeHandler(sessionId) {
                var phoneEl = document.getElementById('registerCodePhoneInput');
                var emailEl = document.getElementById('registerCodeEmailInput');
                var phone = phoneEl ? phoneEl.value.trim() : '';
                var email = emailEl ? emailEl.value.trim() : '';
                if (!phone || !email) {
                    alert(
                        tScan('手机号、邮箱不能为空！', 'Phone and email are required!')
                    );
                    return false;
                }
                if (!isValidLoginEmail(email)) {
                    alert(
                        tScan(
                            '邮箱格式不正确，请重新填写。',
                            'Invalid email format. Please correct it.'
                        )
                    );
                    return false;
                }
                var sid =
                    sessionId ||
                    (cameraQrImg && cameraQrImg.dataset.sessionId) ||
                    '';
                if (!sid) {
                    alert(
                        tScan('请先扫描二维码！', 'Please scan the QR code first!')
                    );
                    return false;
                }
                console.log('[邮件登录] 将发送确认链接到:', email, 'session:', sid);
                try {
                    ...
                    if (!response.ok) {
                        ...
                        console.error('[邮件登录] 发送失败:', errorText);
                        var failDetail = '';
                        try {
                        ...
                        var failMsg = failDetail
                            ? tScan('发送确认邮件失败：', 'Failed to send confirmation email: ') +
                              failDetail
                            : tScan(
                                  '发送确认邮件失败，请稍后重试（HTTP ' + response.status + '）',
                                  'Failed to send confirmation email. Please try again later (HTTP ' +
                                      response.status +
                                      ').'
                              );
                    ...
                    registerEmailVerificationBind = {
                        phone: String((sentData && sentData.phone) || phone).trim(),
                        email: String((sentData && sentData.email) || email).trim(),
                        username: String((sentData && sentData.username) || '').trim()
                    };
                    registerEmailVerificationCode = 'magic_link';
                    setRegisterLoginStatus(
                        'code',
                        tScan(
                            '确认邮件已发送，请在邮箱中点击确认登录',
                    ...
                    console.error('发送确认邮件异常:', err);
                    setRegisterLoginStatus(
                        'code',
                        tScan(
                            '发送确认邮件网络异常：' + String((err && err.message) || err),
                            'Network error while sending confirmation email: ' +
                    ...
                    alert(
                        tScan(
                            '网络异常，确认邮件可能未发出，请重试。',
                            'Network error. The confirmation email may not have been sent. Please try again.'
                        )
                    );
                    return false;
                }
            ...
            async function completeRegisterCodeLoginFromVerifiedEmail() {
                if (registerCodeSubmitting) return;
                var bind = registerEmailVerificationBind || {};
                // 以登录窗口当前填写的手机号/邮箱为准，再与 KV 核对是否一致
                var phoneEl = document.getElementById('registerCodePhoneInput');
                var emailEl = document.getElementById('registerCodeEmailInput');
                var phone = phoneEl ? phoneEl.value.trim() : '';
                var email = emailEl ? emailEl.value.trim() : '';
                if (!phone) phone = String(bind.phone || '').trim();
                if (!email) email = String(bind.email || '').trim();
                var username = String(bind.username || '').trim();
                ...
                try {
                    ...
                    // 用户名以服务端反查结果为准；未带回用户名时不强制核对表单
                    if (username && result.username_matches === false) {
                    ...
                    var displayName =
                        username ||
                        String(result.stored_username || '').trim() ||
                        phone;

                    var pwdFromKv =
                        (result &&
                            result.user_data &&
                            result.user_data.pwd != null &&
                            String(result.user_data.pwd)) ||
                        '';
                    if (result.is_superuser === true) {
                        ...
                        setRegisterLoginStatus('code', '邮箱已确认，还需要密码验证', 'pending');
                        alert('你还需要密码验证！');
            ...
            async function registerCodeLoginSubmit() {
                if (registerEmailVerificationCode === 'magic_link') {
                    alert('请到邮箱点击「确认登录」按钮，电脑端会自动完成登录。');
                    return;
                }
                if (registerCodeSubmitting) return;
                ...
                registerEmailVerificationBind.username = username;
                await completeRegisterCodeLoginFromVerifiedEmail();
            }
            ...
            function openRegister() {
                ...
                registerUiGeneration += 1;
                stopEmailLoginConfirmPoll();
                resetEmbeddedScanLoginState();
                registerScanPhoneMismatchCount = 0;
                registerLastHandledPhoneMismatchAt = 0;
