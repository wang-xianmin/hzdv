(function () {
  ...
  /** 企业问答修订：与网站背景 / 产品目录同权 */
  window.userCanSeeOpsAgentQa = function (roleOrMask) {
    return window.userCanSeeOpsCatalog(roleOrMask);
  };

  /** 发布栏：超管 | 技术员 | 内容审核主管 | 内容审核员 */
  window.userCanSeeOpsDocs = function (roleOrMask) {
    return (resolveTypeMask(roleOrMask) & window.USER_TYPE_OPS_HERO) !== 0;
  };
  ...
  window.syncOpsMenuVisibility = function () {
    ...
    var showAgentQa =
      loggedIn &&
      (typeof window.userCanSeeOpsAgentQa === "function"
        ? window.userCanSeeOpsAgentQa()
        : showCatalog);
    var showDocs =
      loggedIn &&
      (typeof window.userCanSeeOpsDocs === "function"
        ? window.userCanSeeOpsDocs()
        : showCatalog);
    var showWebRefine =
      loggedIn &&
    ...
    setOpsItemVisible(document.getElementById("topNavAgentQa"), showAgentQa);
    setOpsItemVisible(document.getElementById("topNavOpsDocs"), showDocs);
