(function () {
  ...
  /** 口译器：临时登录可用；正式收紧为超管|A类|B类 */
  window.TRANSLATOR_TEMP_OPEN_TO_ANY_LOGIN = true;
  window.USER_TYPE_TRANSLATOR =
    window.USER_TYPE_SUPERUSER | window.USER_TYPE_UA | window.USER_TYPE_UB;
  ...
  window.userCanUseTranslator = function (roleOrMask) {
    if (window.TRANSLATOR_TEMP_OPEN_TO_ANY_LOGIN) {
      ...
      if (window.getCurrentUserPhone && window.getCurrentUserPhone()) return true;
    }
    var mask = 0;
