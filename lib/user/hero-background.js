(function () {
  ...
  function clearVideoFreeze(video) {
    ...
    video.dataset.heroAdvanceRequested = "";
  }
  ...
  function bindHoldLastFrame(video) {
    ...
    function handleVideoComplete() {
      if (!video || video.dataset.heroAdvanceRequested === "1") return;
      var active = state.layers[state.activeLayer];
      if (!active || !active.contains(video)) return;

      if (state.items.length <= 1 || state.reducedMotion) {
        freezeVideoOnLastFrame(video);
        return;
      }
      var item = state.items[state.index];
      if (itemDisplayKind(item) !== "video") {
        freezeVideoOnLastFrame(video);
        return;
      }

      video.dataset.heroAdvanceRequested = "1";
      freezeVideoOnLastFrame(video);
  ...
  /** 图片：按设定间隔；视频：等播完（由 ended 触发），此处只做异常兜底 */
  function scheduleNext() {
    ...
    var item = state.items[state.index];
    if (itemDisplayKind(item) === "video") {
      var active = getLayerMediaEl(state.layers[state.activeLayer]);
      var fallbackMs = 180000;
      if (active && active.tagName === "VIDEO") {
        var d = Number(active.duration);
        if (isFinite(d) && d > 0.5) {
          var remain = Math.max(0, d - (Number(active.currentTime) || 0));
          fallbackMs = Math.ceil(remain * 1000) + 4000;
        }
      }
      state.timer = setTimeout(function () {
        showItem(nextIndex(), true);
