...
(function (global) {
  ...
  function ensureDom() {
    ...
    fileInput.type = "file";
    fileInput.accept = "image/*,.png,.jpg,.jpeg,.webp,.bmp,.gif";
      ...
      fetch("/api/ocr", { method: "POST", body: fd })
        ...
        .then(function (pack) {
          ...
          if (inputEl && !String(inputEl.value || "").trim()) {
            inputEl.value = t("请根据这张图的文字回答：\n", "Based on this OCR text, answer:\n") + text;
            syncSendState();
          }
        })
        .catch(function (err) {
          appendAssistant(
            t("识别请求失败：", "OCR request failed: ") + String((err && err.message) || err)
          );
        });
    }
