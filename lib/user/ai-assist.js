/**
 * AI 助手：顶栏入口 → 底部 agent → 紧凑对话窗口（约 420px 宽，不做大弹窗）
 */
(function (global) {
  "use strict";
  ...
  function appendMessage(role, text) {
    messages.push({ role: role, text: String(text || "") });
    renderThread();
  }
  ...
  function submitPrompt(text) {
    ...
    setTimeout(function () {
      appendAssistant(
        t(
          "已收到你的问题。对话能力即将接入，窗口会保持这个紧凑尺寸。",
          "Got it. Chat will stay in this compact window once the agent is connected."
        )
      );
    }, 280);
  }
