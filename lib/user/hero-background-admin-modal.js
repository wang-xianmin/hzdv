(function () {
  ...
  function convertCaptureThenMaybeAudio(file, convertFn) {
    return convertFn(file).then(function (out) {
      return restoreAudioFromOriginal(file, out);
    });
  }
  ...
  function convertViaVideoCaptureStream(file) {
    return new Promise(function (resolve, reject) {
      ...
      function startRecord() {
        ...
        // 优先用 Web Audio 抽轨（静音预览时仍有声）；否则用 captureStream 自带音轨
  ...
  function convertWithFfmpeg(file, options) {
    var opts = options || {};
    var stabilize = opts.stabilize !== false;
    ...
    preview.mountMessage(
      stabilize ? "正在转码并减轻手抖（稳像，不是插帧）…" : "正在后台重编码为 mp4…"
    );

    // 稳像 + 固定 30fps。绝不要“每半秒插一帧”——那只会更卡。
  ...
  function ensureFinalMp4(file) {
    if (/\.webm$/i.test(file.name || "") || /webm/i.test(file.type || "")) {
      ...
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: true });
    }
    if (needsMp4Convert(file)) {
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: true });
    }
    // 已是 mp4：直接用，不再强制稳像重编码（会卡很久且进度像不动）
  ...
  function ensureMp4File(file) {
    ...
    var apple = isAppleMovLike(file);
    var steps = apple || isSafariBrowser()
      ? [
          function () {
