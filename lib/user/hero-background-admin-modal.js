(function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      function startRecord() {
        ...
        video.play().then(function () {
          ...
          reject(new Error("无法自动播放以转码（请保持屏幕常亮后重试）"));
        });
  ...
  function convertViaCanvasPump(file) {
    return new Promise(function (resolve, reject) {
      ...
      var failTimer = setTimeout(function () {
        cleanup();
        reject(new Error("画布转码超时"));
      }, maxMs);

      video.onerror = function () {
        clearTimeout(failTimer);
        cleanup();
        reject(new Error("无法解码视频供画布绘制"));
      };
      ...
      video.onloadeddata = function () {
        ...
        // 小窗里同时显示源视频；canvas 仅用于编码
        preview.mountMessage("正在逐帧编码…");
        ...
        function draw() {
          ...
          try {
            ...
            preview.setProgress(video.currentTime, video.duration);
          } catch (eD) {}
  ...
  /** webm → mp4；手机片再尽量稳像 */
  function ensureFinalMp4(file, sourceFile) {
    var fromApple = isAppleMovLike(sourceFile || file) || isSafariBrowser();
    if (/\.webm$/i.test(file.name || "") || /webm/i.test(file.type || "")) {
      setStatus("中间格式为 webm，继续转为 mp4 并稳像…");
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: true });
    }
    if (!needsMp4Convert(file) && fromApple) {
      setStatus("mp4 已就绪，正在稳像减轻手抖…");
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: true }).catch(function (err) {
        console.warn("[hero-bg] stabilize skipped", err);
        setStatus("稳像跳过，使用已转好的 mp4", "ok");
        return file;
      });
    }
    if (needsMp4Convert(file)) {
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: fromApple });
    }
  ...
  /**
   * 手机直传优化：多路径串行，尽量把 MOV/HEVC 变成可上网站的 mp4。
   * 失败才抛错，禁止上传原 mov。
   */
  function ensureMp4File(file) {
    if (!file || !isVideoFile(file)) return Promise.resolve(file);
    if (!needsMp4Convert(file)) {
      if (isAppleMovLike(file) || isSafariBrowser()) {
        setStatus("视频已是 mp4，尝试稳像减轻手抖…");
        return convertWithFfmpeg(file, { forceReencode: true, stabilize: true }).catch(function () {
          return file;
        });
      }
      return Promise.resolve(file);
    ...
    var steps = apple || isSafariBrowser()
      ? [
          function () {
            return convertViaVideoCaptureStream(file);
          },
          function () {
            return convertViaCanvasPump(file);
          },
          function () {
            return convertWithFfmpeg(file, { forceReencode: true, stabilize: true });
          },
    ...
    return chain
      .then(function (out) {
        return ensureFinalMp4(out, file);
      })
      .then(function (out) {
