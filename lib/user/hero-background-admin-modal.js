(function () {
          ...
          '<div class="hero-bg-admin-bottom">' +
            '<div class="hero-bg-admin-bottom-head">' +
              '<div class="hero-bg-admin-bottom-head-left">' +
                ...
                '<label class="hero-bg-admin-mute-opt" title="转视频格式时默认静音；取消勾选则保留原声（更慢）">' +
  ...
  /** 边播边转时是否带声：静音模式下 captureStream 不录音轨 */
  function applyConvertMuteState(video) {
    if (!video) return;
    if (isConvertMuted()) {
      video.muted = true;
      video.defaultMuted = true;
  ...
  function playForConvert(video) {
    applyConvertMuteState(video);
    return video.play().catch(function () {
      // 带声自动播放失败时退回静音，保证至少能转画面
      video.muted = true;
      video.defaultMuted = true;
      video.setAttribute("muted", "");
      return video.play();
  ...
  function convertViaVideoCaptureStream(file) {
    return new Promise(function (resolve, reject) {
      ...
      function startRecord() {
        ...
        playForConvert(video).then(function () {
          function tick() {
            if (cleaned) return;
            if (video.ended) return;
            if (video.paused && recorder && recorder.state === "recording") {
              playForConvert(video).catch(function () {});
            }
