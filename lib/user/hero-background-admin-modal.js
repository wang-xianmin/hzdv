(function () {
      ...
      .then(function (mods) {
        ...
        return Promise.all([
          ...
          return ffmpeg.load({ coreURL: urls[0], wasmURL: urls[1] }).then(function () {
            ffmpegBundle = { ffmpeg: ffmpeg, fetchFile: fetchFile };
  ...
  /**
   * iPhone MOV 等 → H.264 MP4（浏览器内 ffmpeg.wasm，不走不可靠的 MediaRecorder）
   */
  function ensureMp4File(file) {
    ...
    if (sizeMb > 120) {
      setStatus(
        "视频约 " + sizeMb.toFixed(0) + "MB，浏览器转码可能内存不足。建议先压到 100MB 内，或手机改为「兼容性最高」拍摄后再传",
        "error"
      );
    }

    var inName = "input" + (/\.[a-z0-9]+$/i.test(file.name || "") ? file.name.replace(/^.*(\.[a-z0-9]+)$/i, "$1") : ".mov");
    var outName = "output.mp4";
    var baseOut = String(file.name || "clip").replace(/\.[^.]+$/, "") || "clip";

    return loadFfmpegBundle()
      .then(function (bundle) {
        var ffmpeg = bundle.ffmpeg;
        var fetchFile = bundle.fetchFile;
        setStatus("正在写入视频…");
        return fetchFile(file).then(function (data) {
          return ffmpeg.writeFile(inName, data);
        }).then(function () {
          var isAppleContainer =
            /\.(mov|qt)$/i.test(inName) || /quicktime/i.test(file.type || "");
          // iPhone MOV 常见为 HEVC：直接 copy 进 mp4 网站仍可能播不了，一律重编码
          if (isAppleContainer) return null;
          setStatus("尝试快速封装为 mp4…");
          return ffmpeg
            .exec(["-i", inName, "-c", "copy", "-movflags", "+faststart", outName])
            .then(function () {
              return ffmpeg.readFile(outName).then(function (out) {
        ...
        }).then(function (copiedOut) {
          if (copiedOut) return copiedOut;
          setStatus("正在重编码为 H.264 mp4（可能需 1–几分钟，请勿关闭页面）…");
          return ffmpeg
            .deleteFile(outName)
            .catch(function () {})
            .then(function () {
      ...
      .then(function (result) {
        if (!result || !result.data || !result.data.length) {
          throw new Error("转码结果为空");
        }
        var bytes = result.data.buffer
          ? result.data
          : new Uint8Array(result.data);
        var outFile = new File([bytes], baseOut + ".mp4", { type: "video/mp4" });
        setStatus(
          result.via === "copy" ? "已封装为 mp4" : "已转为 H.264 mp4",
          "ok"
        );
        return outFile;
      })
      .catch(function (err) {
        ...
        setStatus(
          "自动转 mp4 失败（iPhone 高效/HEVC 视频常见）。请到 设置→相机→格式→选「兼容性最高」重拍后上传；或电脑导出 mp4 再拖入。将暂按原格式上传。",
          "error"
        );
        return file;
      });
