(function () {
  ...
  function openConvertPreview(titleText) {
    ...
    wrap.innerHTML =
      ...
      '<div class="hero-bg-convert-preview__hint">正在边播边转（可能出声），请勿离开本页</div>' +
      "</div>";
  ...
  /** 边播边转时尽量带声：静音 captureStream 录出来的音轨是空的 */
  function unmuteForCapture(video) {
    if (!video) return;
    video.muted = false;
  ...
  /**
   * MediaRecorder 画面转完后，用原片音轨补声（只重编码音频，很快）。
   * 失败则退回无声/弱声成片，不阻断上传。
   */
    ...
    return loadFfmpegBundle()
      ...
      .then(
        ...
        function (err) {
          ...
          setStatus("补声未成功，保留已转画面（可能无声）");
          return convertedFile;
        }
      );
  }

  function convertCaptureThenRestoreAudio(file, convertFn) {
  ...
  function ensureMp4File(file) {
    ...
    var steps = apple || isSafariBrowser()
      ? [
          function () {
            return convertCaptureThenRestoreAudio(file, convertViaVideoCaptureStream);
          },
          function () {
            return convertCaptureThenRestoreAudio(file, convertViaCanvasPump);
          },
          function () {
            return convertWithFfmpeg(file, { forceReencode: true, stabilize: true });
          },
