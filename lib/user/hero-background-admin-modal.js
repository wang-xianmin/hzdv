(function () {
  ...
  function convertWithFfmpeg(file) {
    ...
    var preview = openConvertPreview("FFmpeg 转码中");
    preview.mountMessage("正在后台重编码为 mp4…");

    return loadFfmpegBundle()
      .then(function (bundle) {
        setStatus("正在用 FFmpeg 转为 mp4…");
        return bundle.fetchFile(file).then(function (data) {
          ...
          return bundle
            ...
            .catch(function () {
              preview.mountMessage("正在重编码（可稍等）…");
  ...
  function ensureFinalMp4(file) {
    ...
    if (/\.webm$/i.test(file.name || "") || /webm/i.test(file.type || "")) {
      setStatus("中间格式为 webm，继续封装为 mp4…");
  ...
  function ensureMp4File(file) {
    ...
    // iPhone/Safari：原生优先；其它浏览器：FFmpeg 与原生都试
