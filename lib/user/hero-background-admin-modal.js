(function () {
          ...
          '<div class="hero-bg-admin-bottom">' +
            '<div class="hero-bg-admin-bottom-head">' +
              '<div class="hero-bg-admin-bottom-head-left">' +
                ...
                '<label class="hero-bg-admin-mute-opt" title="转码时预览不外放；成片仍保留原声">' +
                  '<input type="checkbox" id="heroBgConvertMute" checked /> 静音' +
                '</label>' +
              '</div>' +
  ...
  var convertMuteEl = document.getElementById("heroBgConvertMute");
  var slotFileInput = document.getElementById("heroBgSlotFileInput");
  var bodyOverflowPrev = "";
  var pendingSlot = null;
  var focusedSlot = null;

  /** 转格式「静音」= 预览不外放；成片仍尽量保留原声 */
  function isConvertMuted() {
    if (!convertMuteEl) return true;
    return !!convertMuteEl.checked;
  }
  ...
  function loadFfmpegBundle() {
    ...
    var loadTimer = setInterval(function () {
      loadPct = Math.min(18, loadPct + 1);
      setStatus("正在加载转码组件… " + loadPct + "%");
      if (convertPreviewUi) {
        convertPreviewUi.mountMessage("正在加载转码组件（约 25–35MB）…");
        convertPreviewUi.setPercent(loadPct);
      }
    }, 800);
    ...
    function viaEsm() {
      return Promise.all([
        ...
        ffmpeg.on("progress", function (ev) {
          ...
          if (convertPreviewUi) convertPreviewUi.setPercent(pct);
        });
    ...
    ffmpegLoadPromise = (isSafariBrowser() ? viaUmd().catch(viaEsm) : viaEsm().catch(viaUmd))
      .then(function (bundle) {
        ...
        if (convertPreviewUi) convertPreviewUi.setPercent(20);
        return bundle;
      })
      .catch(function (err) {
        clearInterval(loadTimer);
  ...
  function isAppleMovLike(file) {
    if (!file) return false;
    if (/quicktime|video\/hevc|video\/x-m4v/i.test(file.type || "")) return true;
    return /\.(mov|qt|m4v)$/i.test(file.name || "");
  }
  ...
  function pickRecorderMimePreferMp4() {
  ...
  function fileFromRecorderChunks(chunks, mime, baseName) {
    var isMp4 = /mp4/i.test(mime || "");
    var blob = new Blob(chunks, { type: isMp4 ? "video/mp4" : mime.split(";")[0] || "video/webm" });
    var ext = isMp4 ? ".mp4" : ".webm";
    return new File([blob], (baseName || "clip") + ext, { type: blob.type });
  }

  var convertPreviewUi = null;
  ...
  /** 转码预览小窗：一边播放一边转，用户能感知进度 */
  function openConvertPreview(titleText) {
    closeConvertPreview();
    var wrap = document.createElement("div");
    wrap.className = "hero-bg-convert-preview";
    wrap.setAttribute("role", "status");
    wrap.innerHTML =
      '<div class="hero-bg-convert-preview__card">' +
      '<div class="hero-bg-convert-preview__title"></div>' +
      '<div class="hero-bg-convert-preview__stage"></div>' +
      '<div class="hero-bg-convert-preview__meta">' +
      '<span class="hero-bg-convert-preview__time">0:00 / 0:00</span>' +
      '<span class="hero-bg-convert-preview__pct">0%</span>' +
      "</div>" +
      '<div class="hero-bg-convert-preview__bar"><i></i></div>' +
      '<div class="hero-bg-convert-preview__hint">正在边播边转，请勿离开本页</div>' +
      "</div>";
    var titleEl = wrap.querySelector(".hero-bg-convert-preview__title");
    var stageEl = wrap.querySelector(".hero-bg-convert-preview__stage");
    var timeEl = wrap.querySelector(".hero-bg-convert-preview__time");
    var pctEl = wrap.querySelector(".hero-bg-convert-preview__pct");
    var barEl = wrap.querySelector(".hero-bg-convert-preview__bar > i");
    titleEl.textContent = titleText || "视频转码中";
    ...
    var ui = {
      ...
      stage: stageEl,
      setTitle: function (t) {
        titleEl.textContent = t || "视频转码中";
      },
      setProgress: function (current, duration) {
        var cur = Number(current) || 0;
        var dur = Number(duration) || 0;
        timeEl.textContent = formatClock(cur) + " / " + formatClock(dur);
        var pct = dur > 0 ? Math.min(99, Math.round((cur / dur) * 100)) : 0;
      ...
      mountVideo: function (video) {
        stageEl.innerHTML = "";
        video.className = "hero-bg-convert-preview__video";
        ...
        tickTimer = setInterval(function () {
          try {
            ui.setProgress(video.currentTime, video.duration);
          } catch (e) {}
        }, 200);
      },
      mountCanvas: function (canvas) {
        stageEl.innerHTML = "";
        canvas.className = "hero-bg-convert-preview__canvas";
        stageEl.appendChild(canvas);
      },
      mountMessage: function (msg) {
        stageEl.innerHTML = "";
        var p = document.createElement("div");
        p.className = "hero-bg-convert-preview__msg";
        p.textContent = msg || "处理中…";
      ...
      close: function () {
        ...
        tickTimer = null;
        detachEl(wrap);
        if (convertPreviewUi === ui) convertPreviewUi = null;
      },
    };
    convertPreviewUi = ui;
    return ui;
  ...
  function closeConvertPreview() {
    if (convertPreviewUi) convertPreviewUi.close();
    convertPreviewUi = null;
  }

  function attachPreviewVideo(video, preview) {
    // iOS 需要在 DOM 中；这里放进可见小窗，边播边转
    if (preview && preview.mountVideo) preview.mountVideo(video);
    else {
      video.style.cssText =
        "position:fixed;left:0;top:0;width:160px;height:90px;opacity:0.02;pointer-events:none;z-index:-1;";
  ...
  /**
   * 用 Web Audio 抽音轨：元素保持有声供编码，「静音」只是不接到扬声器。
   * 若直接 video.muted=true，MediaRecorder 录到的音轨会是空的。
   */
  function createConvertAudioTap(video) {
    var AudioCtx = window.AudioContext || window.webkitAudioContext;
    if (!AudioCtx || !video) return null;
    try {
      ...
      var tap = {
        ctx: ctx,
        dest: dest,
        syncSpeakers: function () {
          try {
            source.disconnect();
          } catch (eDisc) {}
          try {
            source.connect(dest);
            if (!isConvertMuted()) source.connect(ctx.destination);
          } catch (eConn) {}
        },
        addTracksTo: function (stream) {
          if (!stream || !dest) return 0;
          var n = 0;
      ...
      console.warn("[hero-bg] audio tap unavailable", eTap);
      return null;
    }
  }

  /** 预览外放开关；成片录音时优先保持元素不 muted */
  function applyConvertMuteState(video, audioTap) {
    if (!video) return;
    if (audioTap) {
      // 有音频图：元素保持有声，静音只断扬声器
      video.muted = false;
      ...
      audioTap.syncSpeakers();
      return;
    }
    // 无音频图：只能靠 muted 控外放（成片可能无声，后续会尝试补声）
    if (isConvertMuted()) {
      video.muted = true;
      video.defaultMuted = true;
  ...
  function playForConvert(video, audioTap) {
    applyConvertMuteState(video, audioTap);
    var prep = Promise.resolve();
    if (audioTap && audioTap.ctx && audioTap.ctx.state === "suspended") {
      prep = audioTap.ctx.resume().catch(function () {});
    }
  ...
  /**
   * MediaRecorder 画面转完后，若捕获时没带上音轨，用原片补声。
   * 「静音」只影响预览外放，不再跳过补声。
   */
  function restoreAudioFromOriginal(srcFile, convertedFile) {
    if (!srcFile || !convertedFile) return Promise.resolve(convertedFile);
    if (convertedFile.__heroHadAudio) return Promise.resolve(convertedFile);

    var preview = openConvertPreview("补回声音");
    preview.mountMessage("画面已就绪，正在写入原声…");
    setStatus("正在补回视频声音…");
    var srcExt = /\.[a-z0-9]+$/i.test(srcFile.name || "")
      ? srcFile.name.replace(/^.*(\.[a-z0-9]+)$/i, "$1")
      : ".mov";
    var inVideo = "picture.mp4";
    var inSrc = "source" + srcExt;
    var outName = "with-audio.mp4";
    var baseOut = String(srcFile.name || convertedFile.name || "clip").replace(/\.[^.]+$/, "") || "clip";
    var settled = false;
    var tick = 0;
    var progressTimer = setInterval(function () {
      tick += 1;
      preview.setPercent(Math.min(90, tick * 3));
    }, 1000);

    var work = loadFfmpegBundle()
      .then(function (bundle) {
        preview.mountMessage("转码组件已加载，正在混合音轨…");
        return Promise.all([
          bundle.fetchFile(convertedFile).then(function (d) {
            return bundle.write(inVideo, d);
          }),
          bundle.fetchFile(srcFile).then(function (d) {
            return bundle.write(inSrc, d);
          }),
        ])
          .then(function () {
            return bundle.del(outName).then(function () {
              return bundle.exec([
                "-i",
                inVideo,
                "-i",
          ...
          .then(function (data) {
            return Promise.all([
              ...
              if (!data || !data.length) throw new Error("补音输出为空");
              var bytes = data.buffer ? data : new Uint8Array(data);
              var fileOut = new File([bytes], baseOut + ".mp4", { type: "video/mp4" });
              fileOut.__heroHadAudio = true;
              return fileOut;
            });
          });
      });
    ...
    return Promise.race([work, timeout])
      .then(
        function (fileOut) {
          if (settled) return convertedFile;
          settled = true;
          clearInterval(progressTimer);
          closeConvertPreview();
          setStatus("声音已写入成片", "ok");
          return fileOut;
        },
        function (err) {
          if (settled) return convertedFile;
          ...
          clearInterval(progressTimer);
          closeConvertPreview();
          console.warn("[hero-bg] restore audio failed", err);
          setStatus("补声未成功，成片可能无声");
          return convertedFile;
        }
      );
  }

  function convertCaptureThenMaybeAudio(file, convertFn) {
    // 边播边转已尽量写入音轨；不再二次加载 FFmpeg 补声（会卡在 0% 几分钟）
    return convertFn(file);
  }

  /**
   * 路径1：挂载 DOM 的 video.captureStream + MediaRecorder（Safari/iPhone 解 HEVC 最稳）
   */
  function convertViaVideoCaptureStream(file) {
    return new Promise(function (resolve, reject) {
      var mime = pickRecorderMimePreferMp4();
      if (!mime) {
        reject(new Error("MediaRecorder 不可用"));
        return;
      }
      var preview = openConvertPreview("边播边转 · 原生捕获");
      setStatus("手机视频转码中（小窗预览播放）…");
      var url = URL.createObjectURL(file);
      var video = document.createElement("video");
      video.playsInline = true;
      video.setAttribute("playsinline", "");
      video.setAttribute("webkit-playsinline", "");
      video.preload = "auto";
      attachPreviewVideo(video, preview);
      video.src = url;
      ...
      var maxMs = Math.max(180000, Math.min(600000, (file.size / (1024 * 1024)) * 20000));
      var failTimer = setTimeout(function () {
        fail(new Error("原生捕获转码超时"));
      }, maxMs);
      ...
      function fail(err) {
        ...
        cleanup();
        reject(err instanceof Error ? err : new Error(String(err || "原生捕获失败")));
      }

      function succeed(out) {
        if (settled) return;
        settled = true;
        clearTimeout(failTimer);
        if (out) out.__heroHadAudio = !!hadAudio;
        cleanup();
        resolve(out);
      }

      video.onerror = function () {
        fail(new Error("浏览器无法解码该视频"));
      };

      function startRecord() {
        // 先抓画面轨，再建 AudioContext（部分浏览器先建音频图会导致 captureStream 无视频轨）
        try {
          video.muted = false;
          video.defaultMuted = false;
          video.removeAttribute("muted");
        ...
        if (!stream || !stream.getVideoTracks().length) {
          fail(new Error("无法捕获视频轨"));
          return;
        }
        audioTap = createConvertAudioTap(video);
        applyConvertMuteState(video, audioTap);
        if (audioTap) {
          ...
          if (audioTap.addTracksTo(stream) > 0) hadAudio = true;
        ...
        recorder.onstop = function () {
          ...
          var out = fileFromRecorderChunks(chunks, mime, base);
          if (!out.size) {
            fail(new Error("转码结果为空"));
            return;
          }
          succeed(out);
        };
        ...
        stallTimer = setInterval(function () {
          if (cleaned || video.ended) return;
          var t = Number(video.currentTime) || 0;
          if (t > lastTime + 0.05) {
            lastTime = t;
            lastProgressAt = Date.now();
          } else if (Date.now() - lastProgressAt > 8000) {
            fail(new Error("原生捕获进度停滞"));
          }
        }, 1000);

        playForConvert(video, audioTap).then(function () {
          function tick() {
            if (cleaned) return;
            if (video.ended) return;
            if (video.paused && recorder && recorder.state === "recording") {
              playForConvert(video, audioTap).catch(function () {});
            }
            raf = requestAnimationFrame(tick);
          }
          raf = requestAnimationFrame(tick);
        }).catch(function () {
          fail(new Error("无法自动播放以转码（请保持屏幕常亮后重试）"));
        });
  ...
  /**
   * 路径2：canvas 逐帧绘制 + canvas.captureStream（video.captureStream 不可用时）
   * 注意：小窗必须一直挂着 video 播放；切勿用 mountMessage 把 video 从 DOM 摘掉，
   * 否则 Safari/iOS 会停播，进度永远停在 0%。
   */
  function convertViaCanvasPump(file) {
    return new Promise(function (resolve, reject) {
      var mime = pickRecorderMimePreferMp4();
      if (!mime || typeof HTMLCanvasElement === "undefined") {
        reject(new Error("Canvas 录制不可用"));
        return;
      }
      if (typeof HTMLCanvasElement.prototype.captureStream !== "function") {
        reject(new Error("canvas.captureStream 不可用"));
        return;
      }

      var preview = openConvertPreview("边播边转 · 画布模式");
      setStatus("手机视频转码中（小窗预览）…");
      var url = URL.createObjectURL(file);
      var video = document.createElement("video");
      video.playsInline = true;
      video.setAttribute("playsinline", "");
      ...
      function fail(err) {
        ...
        clearTimeout(failTimer);
        cleanup();
        reject(err instanceof Error ? err : new Error(String(err || "画布转码失败")));
      }
      ...
      video.onloadeddata = function () {
        ...
        // 保持小窗播放源视频；canvas 仅后台喂给 MediaRecorder
        audioTap = createConvertAudioTap(video);
        applyConvertMuteState(video, audioTap);

        var fps = 30;
        var stream = canvas.captureStream(fps);
        canvasTrack = stream.getVideoTracks()[0] || null;
        try {
          if (audioTap && audioTap.addTracksTo(stream) > 0) {
            hadAudio = true;
          } else {
            var vstream =
        ...
        function paintFrame() {
          ...
          try {
            ...
            var t = Number(video.currentTime) || 0;
            preview.setProgress(t, video.duration);
            if (t > lastTime + 0.05) {
              lastTime = t;
              lastProgressAt = Date.now();
            }
  ...
  function convertWithFfmpeg(file, options) {
    var opts = options || {};
    // 默认不做稳像：deshake 在手机 WASM 上极慢，进度会长时间停在 0%
    var stabilize = !!opts.stabilize;
    var forceReencode = !!opts.forceReencode;
    var inName =
      "input" +
    ...
    var preview = openConvertPreview(stabilize ? "转码 + 稳像中" : "FFmpeg 转码中");
    preview.mountMessage(
      stabilize ? "正在转码并减轻手抖（稳像）…" : "正在加载/重编码为 mp4…"
    );
    preview.setPercent(1);

    var vfStable = "deshake,fps=30,scale='min(1280,iw)':-2";
    var vfPlain = "fps=30,scale='min(1280,iw)':-2";

    function runReencode(bundle, vf) {
      preview.mountMessage(
        vf.indexOf("deshake") >= 0 ? "稳像重编码中，请稍候…" : "重编码中，请稍候…"
      );
      var args = [
        "-i",
        inName,
      ...
      return bundle.del(outName).then(function () {
        return bundle.exec(args);
      }).then(function () {
        return bundle.read(outName).then(function (out) {
          return { data: out, via: "reencode" };
        });
      });
    }

    return loadFfmpegBundle()
      .then(function (bundle) {
        setStatus(stabilize ? "正在转码并稳像…" : "正在用 FFmpeg 转为 mp4…");
        preview.mountMessage("组件已就绪，写入文件…");
        preview.setPercent(22);
        return bundle.fetchFile(file).then(function (data) {
          preview.setPercent(28);
          return bundle.write(inName, data);
        }).then(function () {
          preview.setPercent(32);
          if (forceReencode || stabilize) {
            return runReencode(bundle, stabilize ? vfStable : vfPlain).catch(function () {
              return runReencode(bundle, vfPlain);
            });
          }
          return bundle
            .exec(["-i", inName, "-c", "copy", "-movflags", "+faststart", outName])
            .then(function () {
              return bundle.read(outName);
            })
            .then(function (out) {
              if (out && out.length > 1000) return { data: out, via: "copy" };
          ...
          return Promise.all([bundle.del(inName), bundle.del(outName)]).then(function () {
            if (!result || !result.data || !result.data.length) throw new Error("FFmpeg 输出为空");
            var bytes = result.data.buffer ? result.data : new Uint8Array(result.data);
            var fileOut = new File([bytes], baseOut + ".mp4", { type: "video/mp4" });
            fileOut.__heroHadAudio = true;
            return fileOut;
          });
  ...
  /** webm → mp4；已是 mp4 则不再二次重编码（避免卡住） */
  function ensureFinalMp4(file) {
    if (/\.webm$/i.test(file.name || "") || /webm/i.test(file.type || "")) {
      setStatus("中间格式为 webm，继续转为 mp4…");
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: false });
    }
    if (needsMp4Convert(file)) {
      return convertWithFfmpeg(file, { forceReencode: true, stabilize: false });
    }
    return Promise.resolve(file);
  }

  function validateConvertedMp4(srcFile, outFile) {
    return Promise.all([probeVideoDuration(srcFile), probeVideoDuration(outFile)]).then(
      function (pair) {
        var inDur = pair[0];
        var outDur = pair[1];
        if (inDur > 2 && outDur > 0 && outDur < inDur * 0.45) {
          throw new Error(
            "转码时长异常（" + outDur.toFixed(1) + "s / 原 " + inDur.toFixed(1) + "s）"
          );
        }
        if (!outFile || !outFile.size) throw new Error("转码文件为空");
        if (needsMp4Convert(outFile)) throw new Error("结果仍不是 mp4");
        return outFile;
      }
    );
  }
  ...
  function ensureMp4File(file) {
    ...
    if (sizeMb > 180) {
      return Promise.reject(
        new Error(
          "视频约 " +
            sizeMb.toFixed(0) +
            "MB，手机浏览器内存可能不够。请剪到 2 分钟内或先压缩后再传"
        )
      );
    }
    ...
    setStatus(
      "检测到 " +
        (formatFromRef(file.name) || file.type || "非mp4") +
        "，开始转为 mp4（手机直传可能需几十秒到几分钟）…"
    );

    // 一律优先边播边转（有进度）；FFmpeg 仅兜底且不做稳像
    var steps = [
      function () {
        return convertCaptureThenMaybeAudio(file, convertViaVideoCaptureStream);
      },
      function () {
        return convertCaptureThenMaybeAudio(file, convertViaCanvasPump);
      },
      function () {
        return convertWithFfmpeg(file, { forceReencode: true, stabilize: false });
      },
    ...
    steps.forEach(function (step, idx) {
      chain = chain.catch(function (prevErr) {
        if (idx > 0) {
          console.warn("[hero-bg] convert step failed", idx, prevErr);
          setStatus("方案 " + idx + " 未成功，尝试方案 " + (idx + 1) + "…");
        }
        return step();
      });
    });
    ...
    return chain
      .then(function (out) {
        return ensureFinalMp4(out);
      })
      .then(function (out) {
        return validateConvertedMp4(file, out);
      })
      .then(function (out) {
        setStatus("已转为 mp4（" + (out.size / (1024 * 1024)).toFixed(1) + "MB），准备上传…", "ok");
        return out;
      })
      .catch(function (err) {
        console.warn("[hero-bg] all convert paths failed", err);
        closeConvertPreview();
        var tip =
          (err && err.message ? err.message + "。" : "") +
          "已取消上传。建议：设置→相机→格式→「兼容性最高」后重拍再传；或把视频发到电脑导出 mp4。";
        setStatus(tip, "error");
        throw new Error(tip);
      });
  }
  ...
  function renderList(items) {
    ...
    items.forEach(function (item, index) {
      ...
      upBtn.type = "button";
      upBtn.className = "hero-bg-admin-icon-btn hero-bg-admin-icon-btn--move";
      upBtn.textContent = "↑";
      upBtn.setAttribute("aria-label", "上移背景 #" + (index + 1));
      upBtn.title = "上移";
      ...
      downBtn.type = "button";
      downBtn.className = "hero-bg-admin-icon-btn hero-bg-admin-icon-btn--move";
      downBtn.textContent = "↓";
      downBtn.setAttribute("aria-label", "下移背景 #" + (index + 1));
      downBtn.title = "下移";
  ...
  function moveItem(id, direction) {
    var phone = getLoggedInPhone();
    setStatus(direction === "up" ? "上移中…" : "下移中…");
  ...
  function openHeroBgAdminModal() {
    if (!overlay) return;
    if (typeof window.userCanManageHeroBackground === "function" && !window.userCanManageHeroBackground()) {
      alert("无网站背景管理权限");
      return;
    }
