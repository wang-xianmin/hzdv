...
(function (global) {
     ...
     * 展示：A1…A6（对应原：泛泛之交…家庭成员）
     */
    var UA_TIER_LEVELS = [
        { label: 'A1', value: 0, aliases: ['泛泛之交'] },
        { label: 'A2', value: 1, aliases: ['弱关系/熟人'] },
        { label: 'A3', value: 2, aliases: ['普通朋友'] },
        { label: 'A4', value: 4, aliases: ['亲密朋友'] },
        { label: 'A5', value: 8, aliases: ['死党'] },
        { label: 'A6', value: 16, aliases: ['家庭成员'] }
    ];
    ...
    function uA_TierLabelToNum(s) {
        ...
        for (var j = 0; j < UA_TIER_LEVELS.length; j++) {
            var item = UA_TIER_LEVELS[j];
            if (item.label === t) return item.value;
            var aliases = item.aliases || [];
            for (var k = 0; k < aliases.length; k++) {
                if (aliases[k] === t) return item.value;
            }
        }
        return null;
    }
    ...
    /** 弹窗/flex 布局下容器尺寸晚于 createGrid，需补一次布局刷新 */
    function scheduleGridLayoutRefresh() {
        ...
        function run() {
            if (!state.api) return;
                try {
                    if (
                        state.opts.sizeColumnsToFit !== false &&
                        typeof state.api.sizeColumnsToFit === 'function'
                    ) {
                        state.api.sizeColumnsToFit();
                    }
                } catch (eFit) {}
            try {
                if (typeof state.api.resetRowHeights === 'function') {
    ...
    global.UserListGrid = {
        ...
        init: function (options) {
            ...
            state.opts = {
                ...
                sizeColumnsToFit: options.sizeColumnsToFit !== false
            };
            ...
            var gridOptions = {
                ...
                /** 两行表头：group 行 + 叶子列行（对齐 L_ENG 紧凑密度） */
            ...
            window.addEventListener('resize', state._onWinResize);
