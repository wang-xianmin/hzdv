...
(function () {
  ...
  function onPaste(ev) {
    ...
    if (imgPick && !isImageFile(file)) {
      uploadFileForRow(imgPick.getAttribute("data-ops-img-drop"), file);
      return;
    }
  ...
  function bindDropZone(el) {
    ...
    el.addEventListener("drop", function (e) {
      ...
      if (imgCell) {
        ...
        if (isImageFile(file)) uploadImageForRow(rid, file);
        else uploadFileForRow(rid, file);
        return;
      }
      uploadNewFile(file);
    });
  ...
  function fileBadgeLabel(item) {
    if (isPdfLike(item)) return "PDF";
    var ext = fileExtName(item && item.original_name);
    if (ext) return ext.toUpperCase().slice(0, 4);
    return "FILE";
  }

  function findIncompleteDraftId() {
    for (var i = 0; i < rowCache.length; i++) {
      if (isIncompleteRow(rowCache[i])) return rowCache[i].id;
    }
    return "";
  }

  function uploadFileForRow(id, file) {
    if (!id || !file) return;
    var row = null;
    for (var i = 0; i < rowCache.length; i++) {
      if (rowCache[i].id === id) {
    ...
    if (row && !canEditItem(row._raw || row)) {
      setStatus(t("无权修改该行", "Cannot edit this row"), "error");
      return;
    }
    ...
    setStatus(t("附件上传中…", "Uploading file…"), "");
    fetch("/api/ops-docs", { method: "POST", body: fd, cache: "no-store" })
      ...
      .then(function (res) {
        ...
        setStatus(t("PDF/附件已挂到本行", "File attached to row"), "ok");
        loadList();
      })
      .catch(function () {
        if (fileInput) fileInput.value = "";
  ...
  function uploadNewFile(file) {
    ...
    if (!file) return;
    /* 有未完成草稿时，PDF/文档挂到该行；图片仍走图形上传 */
    var draftId = findIncompleteDraftId();
    if (draftId) {
      if (isImageFile(file)) {
        uploadImageForRow(draftId, file);
        return;
      }
      uploadFileForRow(draftId, file);
      return;
    }
    var fd = new FormData();
    fd.append("phone", phone);
  ...
  function isIncompleteRow(row) {
    ...
    var noFile = !String(raw.r2_key || row.r2_key || "").trim();
    /* 标题、图片、附件都空 → 未完成 */
    return titleEmpty && noImage && noFile;
  }
  ...
  function imageRenderer(p) {
    ...
    if (p.data.image_r2_key) {
      inner =
        ...
        '" alt="" />';
    } else if (hasFile(p.data)) {
      var badge = fileBadgeLabel(p.data);
      var isPdf = isPdfLike(p.data);
      inner =
        '<span class="ops-docs-file-badge' +
        (isPdf ? " is-pdf" : "") +
        '">' +
        escHtml(badge) +
        "</span>";
      extraAttr = ' data-ops-open-file="' + id + '"';
    } else {
      inner =
    ...
    return (
      ...
      extraAttr +
      ' title="' +
      (hasFile(p.data) && !p.data.image_r2_key
        ? t("点击打开附件；可再拖入图片", "Open file; drop image to add thumbnail")
        : editable
          ? t("拖入/粘贴 PDF 或图片", "Drop/paste PDF or image")
          : "") +
      '">' +
      inner +
      "</button>"
  ...
  function buildGrid(rows) {
    ...
    var gridOptions = {
      ...
      onCellClicked: function (ev) {
        ...
        var openFile = tgel.closest
          ? tgel.closest("[data-ops-open-file]")
          : null;
        if (openFile) {
          openContent(ev.data);
          return;
        ...
        if (pick && imageInput) {
          /* 已有 PDF 徽章时：点开阅读；空 + 才选图 */
          if (pick.getAttribute("data-ops-open-file")) {
            openContent(ev.data);
            return;
          }
