...
(function () {
  ...
  function ensureUi() {
    ...
    overlay.querySelector("#opsDocsAdminHint").textContent = t(
      "点「新增」加一行（自动填序号/时间/发布者）。标题与图片都空时不能再新增。可编辑标题；图形可拖入/粘贴。",
      "Tap Add for a new row (seq/time/publisher filled). Cannot add again while title and image are both empty."
    );
