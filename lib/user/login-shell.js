(function () {
  ...
  var html =
    ...
    "</div>" +
    '<p class="register-qr-hint register-qr-hint--inline">填写上方信息后，用手机相机扫描二维码；随后查收邮件并点击「确认登录」</p>' +
    ...
    "</div>" +
    "</div>" +
    '<div class="register-input-line register-input-line--stack" id="registerEmailConfirmRow">' +
    '<p id="registerEmailConfirmHint" class="register-login-status" style="margin:0;">扫码后请到邮箱点击「确认登录」按钮，无需填写验证码</p>' +
    "</div>" +
    '<div class="register-input-line register-input-line--stack" id="registerSmsCodeRow" hidden>' +
    '<input id="registerSmsCodeInput" class="register-field-input register-field-input--ghost" type="text" inputmode="numeric" autocomplete="one-time-code" placeholder="验证码（兼容）" aria-label="验证码" />' +
    '<span id="registerSendCodeCooldownLabel" class="register-send-code-cooldown"></span>' +
    "</div>" +
    '<button type="button" id="registerLoginCodeBtn" class="register-primary-btn" hidden>登录</button>' +
