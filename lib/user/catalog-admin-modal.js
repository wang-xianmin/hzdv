...
(function () {
  ...
  function ensureUi() {
    ...
    overlay.innerHTML =
      ...
      "</button>" +
      "</div>" +
      '<div class="catalog-admin-case-ext" id="catalogAdminCaseExt" hidden>' +
      '<label class="catalog-admin-field"><span>' +
      t("元信息（每行：键：值，如 公司：汽车制造商）", "Meta (one per line: key: value)") +
      '</span><textarea id="catCaseMeta" rows="3" placeholder="' +
      t("公司：汽车制造商\n行业：汽车\n地区：欧洲", "Company: OEM\nIndustry: Auto\nRegion: Europe") +
      '"></textarea></label>' +
      '<p class="catalog-admin-drawer-hint" style="margin:4px 0 0">' +
      t("—— 总结三卡（前面展示）——", "—— Summary cards (shown first) ——") +
      "</p>" +
      '<label class="catalog-admin-field"><span>' +
      t("挑战 · 总结要点（每行一条）", "Challenge · summary bullets") +
      '</span><textarea id="catCaseSumCh" rows="4"></textarea></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("解决方案 · 总结要点（每行一条）", "Solution · summary bullets") +
      '</span><textarea id="catCaseSumSol" rows="4"></textarea></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("结果 · 总结要点（每行一条）", "Result · summary bullets") +
      '</span><textarea id="catCaseSumRes" rows="4"></textarea></label>' +
      '<p class="catalog-admin-drawer-hint" style="margin:4px 0 0">' +
      t("—— 详细叙述（后面展示）——", "—— Detail narrative (shown after) ——") +
      "</p>" +
      '<label class="catalog-admin-field"><span>' +
      t("顾客 · 正文", "Customer · body") +
      '</span><textarea id="catCaseDetCust" rows="3"></textarea></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("挑战 · 小标题", "Challenge · subtitle") +
      '</span><input type="text" id="catCaseDetChSub" /></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("挑战 · 正文", "Challenge · body") +
      '</span><textarea id="catCaseDetCh" rows="4"></textarea></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("解决方案 · 小标题", "Solution · subtitle") +
      '</span><input type="text" id="catCaseDetSolSub" /></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("解决方案 · 正文", "Solution · body") +
      '</span><textarea id="catCaseDetSol" rows="4"></textarea></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("结果 · 小标题", "Result · subtitle") +
      '</span><input type="text" id="catCaseDetResSub" /></label>' +
      '<label class="catalog-admin-field"><span>' +
      t("结果 · 正文", "Result · body") +
      '</span><textarea id="catCaseDetRes" rows="4"></textarea></label>' +
      '<button type="button" class="catalog-admin-btn" id="catalogAdminCaseSave">' +
      t("保存案例 EXT_JSON", "Save case EXT_JSON") +
      "</button>" +
      "</div>" +
    ...
    overlay.querySelector("#catalogAdminHint").textContent = t(
      "选中一行后：EXT_JSON 按类型编辑（方案四屏 / 产品键值 / 案例总结+详情）；「说明」编辑导语或产品正文。媒体列管理图视频。",
      "Select a row: EXT_JSON by kind (solution / product / case); Description for lead or product prose. Media for files."
    );
    ...
    overlay.querySelector("#catalogAdminSynonymsHint").textContent = t(
      "规范词固定为产品/方案/案例/服务；同义词用逗号或换行分隔。保存后若要向量检索立刻生效，请再点「重建向量索引」。",
      "Canonical kinds: product/solution/case/service; aliases comma/newline separated. After save, rebuild vectors for search to pick up new terms."
    );
    overlay
    ...
    overlay
      .querySelector("#catalogAdminCaseSave")
      .addEventListener("click", saveCaseExt);
    overlay
  ...
  function fillCaseExtForm(item) {
    var cse = (item && item.case) || null;
    if (!cse && item && item.extra_json) {
      try {
        var extra = JSON.parse(String(item.extra_json || "{}"));
        cse = extra.case || {};
      } catch (e) {
    ...
    var meta = cse.meta || {};
    var metaLines = Object.keys(meta)
      .map(function (k) {
        return k + "：" + meta[k];
      })
      .join("\n");
    set("#catCaseMeta", metaLines);
    var sum = cse.summary || {};
    set("#catCaseSumCh", ((sum.challenge && sum.challenge.bullets) || []).join("\n"));
    set("#catCaseSumSol", ((sum.solution && sum.solution.bullets) || []).join("\n"));
    set("#catCaseSumRes", ((sum.result && sum.result.bullets) || []).join("\n"));
    var det = cse.detail || {};
    set("#catCaseDetCust", (det.customer && det.customer.body) || "");
    set("#catCaseDetChSub", (det.challenge && det.challenge.subtitle) || "");
    set("#catCaseDetCh", (det.challenge && det.challenge.body) || "");
    set("#catCaseDetSolSub", (det.solution && det.solution.subtitle) || "");
    set("#catCaseDetSol", (det.solution && det.solution.body) || "");
    set("#catCaseDetResSub", (det.result && det.result.subtitle) || "");
    set("#catCaseDetRes", (det.result && det.result.body) || "");
  }

  function readCaseExtForm() {
    var metaText = (overlay.querySelector("#catCaseMeta") || {}).value || "";
    var meta = {};
    String(metaText)
      .replace(/\r\n/g, "\n")
      .split("\n")
    ...
    return {
      ...
      summary: {
        challenge: {
          ...
          bullets: linesFromTextarea(overlay.querySelector("#catCaseSumCh")),
        },
        solution: {
          title: t("解决方案", "Solution"),
          bullets: linesFromTextarea(overlay.querySelector("#catCaseSumSol")),
        },
        result: {
          title: t("结果", "Result"),
          bullets: linesFromTextarea(overlay.querySelector("#catCaseSumRes")),
        },
      },
      detail: {
        customer: {
          title: t("顾客", "Customer"),
          subtitle: "",
          body: (overlay.querySelector("#catCaseDetCust") || {}).value || "",
        },
        challenge: {
          title: t("挑战", "Challenge"),
          subtitle: (overlay.querySelector("#catCaseDetChSub") || {}).value || "",
          body: (overlay.querySelector("#catCaseDetCh") || {}).value || "",
        },
        solution: {
          title: t("解决方案", "Solution"),
          subtitle: (overlay.querySelector("#catCaseDetSolSub") || {}).value || "",
          body: (overlay.querySelector("#catCaseDetSol") || {}).value || "",
        },
        result: {
          title: t("结果", "Result"),
          subtitle: (overlay.querySelector("#catCaseDetResSub") || {}).value || "",
          body: (overlay.querySelector("#catCaseDetRes") || {}).value || "",
        },
      },
    };
  }
  ...
  function setDrawerTab(tab) {
    var item = findItem(activeItemId);
    var kind = item ? item.kind : "product";
    var isSolution = kind === "solution";
    var isCase = kind === "case";
    ...
    var caseExt = overlay.querySelector("#catalogAdminCaseExt");
    ...
    if (prod) prod.hidden = !(drawerMode === "ext" && !isSolution && !isCase);
    if (caseExt) caseExt.hidden = !(drawerMode === "ext" && isCase);
  ...
  function saveCaseExt() {
    if (!activeItemId) return;
    var item = findItem(activeItemId);
    if (!item) return;
    setStatus(t("保存案例 EXT_JSON…", "Saving case EXT_JSON…"));
    api("PATCH", {
      id: activeItemId,
      kind: "case",
      case: readCaseExtForm(),
    })
      .then(function (pack) {
        if (!pack.ok || !pack.j.item) {
          setStatus(
        ...
        replaceCacheItem(pack.j.item);
        if (gridApi) buildGrid();
        fillCaseExtForm(pack.j.item);
        setStatus(t("案例 EXT_JSON 已保存", "Case EXT_JSON saved"), "ok");
      })
      .catch(function (e) {
        setStatus(String((e && e.message) || e), "error");
      });
  ...
  function openDrawer(id, mode) {
    ...
    if (tab === "ext") {
      if (isSolution) {
        ...
        drawerHintEl.textContent = t(
          "案例 EXT_JSON：元信息 + 挑战/方案/结果总结（前）+ 详细叙述（后）。说明字段只写短导语。",
          "Case EXT_JSON: meta + challenge/solution/result summary first, then detail. Description = short lead only."
        );
      } else {
        drawerHintEl.textContent = t(
      ...
      drawerHintEl.textContent =
        item.kind === "case"
          ? t(
              "案例「说明」只写短导语（列表摘要）；挑战/方案/结果请进 EXT_JSON。",
              "Case Description is a short lead only; put challenge/solution/result in EXT_JSON."
            )
          : t(
