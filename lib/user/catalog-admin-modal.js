...
(function () {
  ...
  function ensureUi() {
    ...
    overlay.querySelector("#catalogAdminDropTip").textContent = t(
      "按顺序上传：第 1 张=缩略图，第 2 张=简述图 · 粘贴/拖入/选文件",
      "Order: #1=thumb, #2=summary · paste / drop / choose file"
    );
  ...
  function openDrawer(id, mode) {
    ...
    drawerHintEl.textContent =
      drawerMode === "solution"
        ? t(
            "填写主展区四屏内容；保存后类型将设为「方案」。媒体第 1 张为缩略图/Hero，第 2 张为简述配图。",
            "Four-screen content; save sets kind to Solution. Media #1 = thumb/hero, #2 = summary."
          )
        : t(
