...
(function () {
  ...
  function ensureUi() {
    ...
    overlay.innerHTML =
      ...
      t("新增产品", "Add product") +
      "</button>" +
      '<button type="button" class="catalog-admin-btn catalog-admin-btn--ghost" id="catalogAdminAddSolution">' +
      t("新增方案", "Add solution") +
      "</button>" +
      ...
      '<label class="catalog-admin-field"><span>' +
      t("主要优势（标题|正文，每行一条）", "Advantages (title|body per line)") +
      '</span><textarea id="catSolAdv" rows="5"></textarea></label>' +
    ...
    overlay.querySelector("#catalogAdminHint").textContent = t(
      "文字改完回车或点出单元格即自动保存；点「媒体」列打开抽屉，可 Ctrl+V 粘贴图片，或拖入/选择图片与视频。表头边缘可拖拽调列宽。",
      "Edits auto-save on Enter or leaving the cell. Open Media for Ctrl+V / drop / file pick. Drag header edges to resize columns."
    );
    ...
    overlay
      .querySelector("#catalogAdminAddSolution")
      .addEventListener("click", addSolutionRow);
    overlay
  ...
  function parseAdvantagesText(text) {
    return String(text || "")
      ...
      .map(function (line) {
        ...
        if (pipe >= 0) {
          return {
            title: s.slice(0, pipe).trim(),
            body: s.slice(pipe + 1).trim(),
          };
        }
        return { title: s, body: "" };
      })
  ...
  function addSolutionRow() {
    ...
    api("POST", {
      action: "create",
      kind: "solution",
      name: t("未命名方案", "Untitled solution"),
      model: "",
      specs: "",
      description: "",
      is_active: 1,
      ...
      .then(function (pack) {
        ...
        cache.push(pack.j.item);
        buildGrid();
        setStatus(t("已新增方案", "Solution created"), "ok");
        openDrawer(pack.j.item.id, "solution");
      })
      .catch(function (e) {
        setStatus(String((e && e.message) || e), "error");
      });
  ...
  function openDrawer(id, mode) {
    ...
    drawerMode = mode === "solution" && isSolution ? "solution" : "media";
    ...
    drawerHintEl.textContent =
      drawerMode === "solution"
        ? t(
            "填写主展区四屏内容；媒体第 1 张为 Hero，第 2 张为简述配图。",
            "Main-area four screens; media #1 = hero, #2 = summary image."
          )
        : t(
