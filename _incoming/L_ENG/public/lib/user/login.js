...
(function () {
    ...
    var topNavAvatarManage = document.getElementById('topNavAvatarManage');
            ...
            function refreshCameraQr() {
                ...
                if (qrTimer) clearInterval(window.qrTimer);
                stopRegisterQrCountdownTimers();
                ...
                getLocalIP().then(function (localIP) {
                    var baseUrl = 'http://' + localIP + ':' + window.location.port;
            ...
            if (topNavUserLogin) {
                topNavUserLogin.addEventListener('click', function () {
                    beginRegisterFlow();
                });
            }
            ...
            if (registerSubmitBtn) {
                registerSubmitBtn.addEventListener('click', async function () {
                    ...
                    try {
                        ...
                        console.error(err);
                        alert('登录失败：' + String((err && err.message) || err || '网络异常'));
                    } finally {
            ...
            async function registerSendEmailCodeHandler() {
                ...
                console.log('%c[验证码登录] 随机6位验证码', 'font-weight:700;font-size:14px;color:#0b7285;', newCode);
                ...
                try {
                    ...
                    console.log('[验证码登录] 已发送至', email, '，验证码:', newCode, '（与邮件正文一致）');
